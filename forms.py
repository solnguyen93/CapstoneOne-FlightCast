...
# Server-side validation
 ...
 # Check if the data contains only letters (no special characters or numbers)
def is_valid_string(form, field):
    data = field.data
    if data is None or not data.isalpha():
...
class FlightForm (FlaskForm):
    ...
    departure_location = StringField('From', validators=[is_valid_string, InputRequired(), Length(min=3, max=30)], render_kw={"placeholder": "Airport code (e.g. SEA, HND, LAX)"})
    arrival_location = StringField('To', validators=[is_valid_string, InputRequired(), Length(min=3, max=30)], render_kw={"placeholder": "Airport code (e.g. SEA, HND, LAX)"})
